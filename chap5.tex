...
Thus, new expressions are also needed for the statistical error uncertainties: 
\begin{equation}
...
The experimental data are corrected using weights defined as $\omega = 1/\text{Acc}$. The 
...
We used a 4-dimensional binning to divide the large phase space available to 
our two particle final state. To evaluate the systematic error 
associated to this correction, we uses the two different binning
presented in table~\ref{tab:AcceptBinning}. The total number of bins is 
constrained by the amount of data generated and must be maintained 
reasonably low in order to keep a good statistical precision and manageable 
bin migration.
...
The electron acceptance, which is necessary in order to correct the number of electrons 
